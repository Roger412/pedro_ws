...
static uint8_t mcp_wait_config(uint32_t timeout_ms)
{
    const uint32_t t0 = HAL_GetTick();
    while ((HAL_GetTick() - t0) < timeout_ms)
    {
        uint8_t canstat = MCP_readRegister(MCP_CANSTAT);

        /* 0xFF = no reply yet, keep waiting                          */
        if (canstat == 0xFF)
        {
            HAL_Delay(2);
            continue;
        ...
        /* CONFIG mode = bits 7-5 == 100b → 0x80                       */
        if ((canstat & 0xE0) == 0x80)
            return 1;

        HAL_Delay(2);  /* give crystal a bit more time                */
    }
    return 0;       /* timed-out                                   */
}
...
//	printf("Loopback test: sent 0x%02X, received 0x%02X\n\r", tx, rx);

    printf("[MCP_reset] Sending RESET instruction...\n\r");
    startSPI();
    SPI_transfer(INSTRUCTION_RESET);
    endSPI();

    if (!mcp_wait_config(1000))        /* waits up to 1000 ms total     */
	{
		printf("[MCP_reset] ERROR: MCP2515 did not reach CONFIG mode\n\r");
		return ERROR_FAIL;
	}

	printf("[MCP_reset] MCP2515 is in CONFIG mode ✓\n\r");

    uint8_t zeros[14];
    memset(zeros, 0, sizeof(zeros));
    printf("[MCP_reset] Clearing TXB registers...\n\r");
    setRegisters(MCP_TXB0CTRL, zeros, 14);
    setRegisters(MCP_TXB1CTRL, zeros, 14);
    setRegisters(MCP_TXB2CTRL, zeros, 14);

    printf("[MCP_reset] Clearing RXB registers...\n\r");
    setRegister(MCP_RXB0CTRL, 0);
    setRegister(MCP_RXB1CTRL, 0);

    printf("[MCP_reset] Setting CANINTE...\n\r");
    setRegister(MCP_CANINTE, CANINTF_RX0IF | CANINTF_RX1IF | CANINTF_ERRIF | CANINTF_MERRF);

    printf("[MCP_reset] Setting RXB0CTRL and RXB1CTRL...\n\r");
    ...
    printf("[MCP_reset] Setting filters...\n\r");
    RXF filters[] = {RXF0, RXF1, RXF2, RXF3, RXF4, RXF5};
    for (int i = 0; i < 6; i++) {
        uint8_t ext = (i == 1);
        CAN_Error result = MCP_setFilter(filters[i], ext, 0);
        if (result != ERROR_OK) {
            printf("[MCP_reset] MCP_setFilter failed on RXF%d, ext=%d, err=%d\n\r", i, ext, result);
            return result;
        }
    }

    printf("[MCP_reset] Setting masks...\n\r");
    MASK masks[] = {MASK0, MASK1};
    for (int i = 0; i < 2; i++) {
        CAN_Error result = MCP_setFilterMask(masks[i], 1, 0);
        if (result != ERROR_OK) {
            printf("[MCP_reset] MCP_setFilterMask failed on MASK%d, err=%d\n\r", i, result);
            return result;
        }
    }

    printf("[MCP_reset] Reset completed successfully.\n\r");
    return ERROR_OK;
}
...
uint8_t MCP_readRegister(uint8_t address)
{
    ...
    uint8_t value = SPI_transfer(0x00);  // Dummy write to receive data
    endSPI();
    return value;
}
